HavenDAoC Server Emulation

# Setting Up a Dev Environment

The following instructions should act as a guide for properly setting up a development environment to fetch and push file changes with GitHub as well as build and run a local DAoC server successfully.

Instructions currently exist regarding setups for both [Ubuntu](#setting-up-on-ubuntu) and [Windows](#setting-up-on-windows) operating systems. These configurations may be performed and a local server run on macOS as well, with some minor alterations--though you'll need either a Windows or wine setup to run the DAoC client.

_**IMPORTANT:** Check the [Environment Requirements](#environment-requirements) section first as you will not be able to complete certain sections without the proper software resources or privileges._

The following sections outline the process of preparing your machine to develop, build, and run a HavenDAoC server:

1. [Environment Requirements](#environment-requirements)
   1.[Considerations for IDEs](#considerations-for-ides)
2. [Setting Up on Ubuntu](#setting-up-on-ubuntu)
   1. [Installing .NET 7.0](#installing-net-70-ubuntu)
...
3. [Setting Up on Windows](#setting-up-on-windows)
   1. [Installing .NET 7.0](#installing-net-70-win)
...
4. [Building Your HavenDAoC Server Locally](#building-your-dol-server-locally)
...
* **Operating System:** Ubuntu or Windows (macOS usable as a server, but cannot run the DAoC game client)
...
* **.NET SDK Framework:** 7.0+ required
...
### Installing .NET 7.0 (Ubuntu)

.NET is an open-source developer platform used for building applications. This repo uses .NET 7.0.X specifically.
...
With the repo on your local hard drive, you need to alter the `serverconfig.xml` file to avoid some errors when building HavenDAoC locally.
...
Now you're ready to [run your own instance of HavenDAoC](#building-your-dol-server-locally)!
...
### Installing .NET 7.0 (Win)

.NET is an open-source developer platform used for building applications. This repo uses .NET 7.0.X specifically, which is supported on all recent versions of Windows.

1. Download the [.NET 7.0 installer](https://dotnet.microsoft.com/download/dotnet/7.0).
...
The RDBMS is installed, but needs a user and database for HavenDAoC to access and use.
...
Prior to accomplishing this step, you will need a recent copy of the `daocDB.sql` file. Without it, you cannot successfully build a local HavenDAoC server. After installing MariaDB, you should also notice a program called HeidiSQL, which you'll need to use for this section.
...
## Building Your HavenDAoC Server Locally

This section provides the commands necessary for both building and running a HavenDAoC server locally.

1. Launch the Terminal or PowerShell, navigate to the `/daoc/` directory and type `dotnet build DOLLinux.sln`. This builds the HavenDAoC server on your machine. <!-- This may take around 1-2 minutes to complete. Don't panic if you see multiple warnings and errors. -->
...
Congratulations! You're now running an instance of HavenDAoC on your machine.
...
The client is ready for a local HavenDAoC server.
...
### Using HavenDAoC Portal

To access your local instance, you'll need the DOL Portal application, which can be downloaded at [DOLServer.net](http://www.dolserver.net/download/file.php?id=3735). Once you've built your HavenDAoC server and it's running locally, you can now access it using the Dawn of Light Portal.
...
The HavenDAoC client launches and creates a new account based on the credentials you entered. You should be brought to the realm selection screen now.
...
When testing your HavenDAoC server in-game, special attention should be paid when utilizing the `/plvl` GM command.
